...
class NdefClass {
  ...
  public:
    ...
    */
    ...
    // Explicitly (re)set the back-pointer that the vendor T5T/T4T/T3T/T2T
    // code dereferences via `ctx->ndef_class_instance` to recover this
    // NdefClass pointer. The constructor already does this, but the vendor
    // library reads from a HARDCODED offset (0x5f8 within ndefContext) that
    // disagrees with main.cpp's view of the struct in our build — likely
    // an ODR-style mismatch where different translation units see different
    // NDEF feature flags and therefore different `subCtx` union sizes.
    //
    // We write to BOTH:
    //   - the symbolic location (correct under no-ODR builds)
    //   - the hardcoded byte offset 0x5f8 (the location the library
    //     actually reads, regardless of main.cpp's struct view)
    //
    // Call once from setup() before the first NFC scan.
    void initBackPointer() {
      // `volatile` casts prevent dead-store elimination on either write.
      *(void * volatile *)((char *)&ctx + 0x5f8) = this;   // library's view
      *(void * volatile *)&ctx.ndef_class_instance = this; // main's view (no-op if same offset)
    }
